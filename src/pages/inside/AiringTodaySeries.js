...
function AiringTodaySeries() {
  ...
  useEffect(() => {
    ...
    if(!isLoading || loadMore) {
      axios.get(`https://api.themoviedb.org/3/tv/airing_today?page=${pageCount}`,{
          params: {
      ...
      .then(res => {
        ...
        setAiringTodaySeries(oldSeries => ([ ...oldSeries, ...data ]));
      })
  ...
  let airingTodaySeriesEl = airingTodaySeries?.map((series) => (
    <div className='w-48'>
      <Link to={`/home/series/${series.id}`}>
        <img className='text-primary w-full' src={`https://image.tmdb.org/t/p/w342${ series.poster_path }`} />
      </Link>
    </div>
  ))
